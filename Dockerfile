...
# tini reaps zombie processes as PID 1; ca-certificates is the trust store
# the CRCON WebSocket client verifies TLS against.
RUN apt-get update \
...
# compose.prod.yaml overrides this to run migrations first. Running the
# server alone is the right default for an orchestrator that migrates in a
# separate job.
CMD ["/app/bin/server"]
